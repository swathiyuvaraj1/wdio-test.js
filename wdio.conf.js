...
let dockerService = Config.dockerService;
let googleChatWebhookUrl = process.env.GOOGLE_CHAT_WEBHOOK;
console.log("Execution started with DOCKER_SERVICE: " + dockerService);
console.log("Google chat webhook url passed: " + googleChatWebhookUrl);
...
exports.config = {
  ...
  //
  hostname: dockerService == "true" ? "selenium-hub" : "localhost",
  port: 4444,
  path: "/wd/hub",
  //
  // ==================
  ...
  //
  maxInstances: dockerService == "true" ? 8 : 1,
  //
  ...
  services: [
    "intercept",
    dockerService == "true" ? "docker" : "chromedriver",
    /*GoogleChatService,
    //package not compatible with wdio v8, getting error
    {
      webhookUrl: "" + googleChatWebhookUrl + "",
      //notifyOnlyOnFailure: false, //Send notification only in case of test failure
      message: "Helloo, its me WDIO",
    },*/
  ],
  ...
  beforeSuite: async function (suite) {
    let suiteName = suite.file.split(path.sep).pop();
    console.log("SuiteName.............." + suiteName);
    console.log("before suite...........");
    if (suiteName != "trial.test.js" && suiteName != "cookie-policy.test.js") {
      await Utils.testStepForReport("LaunchSite:");
